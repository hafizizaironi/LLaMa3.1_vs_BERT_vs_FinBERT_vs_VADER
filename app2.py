...
def get_sentiment_analysis(news):
    prompt = f"News: {news}\nProvide only the sentiment score for the news as either -1 (negative), 0 (neutral), or 1 (positive)."
    ...
    try:
        response = requests.post(
            ...
            json={
                ...
                "messages": [
                    {"role": "system",
                     "content": "You are a sentiment analysis agent. Analyze the given news text and provide a sentiment score (-1 for negative, 0 for neutral, and 1 for positive)."},
        ...
        # Ensure the content is either 1, 0, or -1
        if content == '1':
            return 1
        elif content == '0':
            return 0
...
results_csv_path = 'sentiment_analysis_results_new.csv'
